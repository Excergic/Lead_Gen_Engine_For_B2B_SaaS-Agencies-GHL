...
export default function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadLeads = useCallback(async () => {
    ...
    try {
      const data = await api.leads.list(campaignId);
      setLeads(data);
    } catch {
      // silently ignore — leads table is optional
    } finally {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {campaign ? (
        <>
          ...
          <div className="rounded-xl border border-zinc-800/60 bg-zinc-900/40 p-5 mb-6">
            ...
            {campaign.status === "completed" && (
              <p className="text-xs text-zinc-500 mb-4">
                This campaign is completed and cannot be re-run.
              </p>
            )}
          ...
          {runResult && (
            <div className="rounded-xl border border-emerald-800/50 bg-emerald-500/10 p-5 mb-6">
              ...
              {runResult.drafts_queued > 0 && (
                <Link
                  href="/outreach"
                  className="mt-3 inline-flex text-xs text-violet-400 hover:text-violet-300 underline transition-colors"
                >
          ...
          <div>
            ...
            {leadsLoading ? (
              ...
              <p className="text-sm text-zinc-600 py-4">
                No leads yet. Run the campaign to discover and score leads.
              </p>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-800/60 bg-zinc-900/40">
                      {["Score", "Contact", "Company", "Email", "Status", "Profile"].map((h) => (
                        <th
                          key={h}
